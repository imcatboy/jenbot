import { reputationService, type CreateReputationUserRequest, type ReputationUserResponse, type UpdateReputationUserRequest } from "@/api";
...
export const useReputationUser = (id: number) => {
  return useQuery({
...
export const useCreateReputationUser = () => {
  ...
  return useMutation({
    mutationFn: (request: CreateReputationUserRequest) => reputationService.createReputationUser(request),
...
export const useUpdateReputationUser = () => {
  ...
  return useMutation({
    mutationFn: ({ id, request }: { id: number, request: UpdateReputationUserRequest }) => reputationService.updateReputationUser(id, request),
    onSuccess: (data: ReputationUserResponse) => {
      queryClient.invalidateQueries({ queryKey: REPUTATION_KEYS.all });
      queryClient.setQueryData(REPUTATION_KEYS.user(data.id), data);
    },
