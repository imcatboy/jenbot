...
CANNOT_USE_ACTION_ON_MODERATOR = "❌ Вы не можете использовать это действие на модераторе."
...
USERNAME_OR_REPLY_TO_USER_REQUIRED = "❌ Имя пользователя или ответ на сообщение является обязательным."
SET_REPUTATION_ROLE_MESSAGE = "💬 Выберите роль пользователя."
SET_REPUTATION_DESCRIPTION_MESSAGE = "💬 Введите описание для репутации (до 255 символов)."
...
BAN_WORD_ERROR = "❌ <b>{0}</b> использовал запрещённое слово <tg-spoiler>{1}</tg-spoiler>."
VIOLATIONS = {
...
def get_ban_user_success_message(
    ...
    if expires_at:
        ...
        return BAN_USER_WITHOUT_EXPIRES_AT_SUCCESS.format(escape(username), escape(reason))
...
def get_mute_user_success_message(
    ...
    if expires_at:
        ...
        return MUTE_USER_WITHOUT_EXPIRES_AT_SUCCESS.format(escape(username), escape(reason))
...
def get_violations_message(violations: List[entities.ViolationWithUserEntity]) -> str:
    ...
    for violation in active_violations:
        ...
        message += f"Выдан @{violation.applied_by_user.username}\n"
...
def get_report_message(report: entities.ReportWithUserEntity) -> str:
    ...
    if report.accused_user:
        message += f"Обвиняемый: @{report.accused_user.username} (<code>{report.accused_user.telegram_id}</code>)\n"
    if report.applied_by_user:
        message += f"Изменил статус: @{report.applied_by_user.username} (<code>{report.applied_by_user.telegram_id}</code>)\n"
