...
def get_my_reviews_message(
    ...
    for index, review in enumerate(reviews):
        message += f"{index + 1}. {format_user_handle(review.subject_user.usernames, review.subject_user.telegram_id)}\n"
...
def get_new_review_message(review: entities.ReviewWithRelationsEntity, deleted_by_user: Optional[entities.UserEntity] = None) -> str:
    message = f'<tg-emoji emoji-id="5282973734456565994">💬</tg-emoji> <b>Новый отзыв от {format_user_handle(review.author.usernames, review.author.telegram_id)}</b>\n\n'
    message += f"{format_user_handle(review.subject_user.usernames, review.subject_user.telegram_id)}\n"
    message += f"<b>{REPUTATION_ROLES[review.subject_user.reputation_user.role]}</b> ({review.subject_user.reputation_user.review_count} {get_count_word(review.subject_user.reputation_user.review_count, "отзыв", "отзыва", "отзывов")})\n"
    message += f"<blockquote>{escape(review.message)}</blockquote>\n"
    message += (
    ...
    if deleted_by_user:
        message += f"\n\n<b>Отзыв был удален {format_user_handle(deleted_by_user.usernames, deleted_by_user.telegram_id)}</b>"
...
def get_review_deleted_message(
    ...
    message = f'<tg-emoji emoji-id="5282953552405241953">🔎</tg-emoji> Ваш отзыв на {format_user_handle(review.subject_user.usernames, review.subject_user.telegram_id)} был удален.'
